...
# import grader.questions.product_defs
...
for py in [f[:-3] for f in os.listdir(path) if f.endswith('.py') and f != '__init__.py']:
	mod = import_module('.'.join(['grader.questions', py]))
	classes = [getattr(mod, x) for x in dir(mod) if isinstance(getattr(mod, x), Question)]
	for cls in classes:
		setattr(sys.modules[__name__], cls.__name__, cls)

def main():
	printc(Color.PRP, 'Welcome to the automatic grader')
	...
	for cls in classes:
		...
		question = cls()
		# if not isinstance(question, grader.questions.product_defs.QuestionProductDefs):
		# 	continue
